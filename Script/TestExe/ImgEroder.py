...
if __name__ == "__main__":
    ...
    try:
        ...
        nPixel = "5"
        inputDir = r"D:\v-jiazha\10-share"
        inputFile = os.path.join(inputDir, "mask.pfm")
        outputFile = os.path.join(inputDir, "mask_erode.pfm")
        re = subprocess.run(
            ["ImgEroder", "-in=" + inputFile, "-out=" + outputFile,"-n=" + nPixel], stdout=True, stderr=True, check=True)

        inputFile = os.path.join(inputDir, "mask_erode.pfm")
        outputFile = os.path.join(inputDir, "mask_dilate.pfm")
        re = subprocess.run(
            ["ImgDilator", "-in=" + inputFile, "-out=" + outputFile,"-n=" + nPixel], stdout=True, stderr=True, check=True)
