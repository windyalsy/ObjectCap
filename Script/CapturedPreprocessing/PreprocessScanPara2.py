...
def CallDNGConverter(args):
    re = subprocess.run(["dngconvert","-c","1320,640-2120,1120", args[0]], stdout=False, stderr=True, check=True, cwd=args[1])
...
if __name__ == "__main__":
    ...
    # DEPLOYMENT_ROOT = os.path.join(DATA_ROOT,r'Object')
    ...
    try:
        ...
        for v in range(startView,nviews):
            ...
            if not os.path.isfile(os.path.join(framesDirectory, r"result_c0223.pfm")):
                # downsample pfm
                logging.info("downsample pfm row...")
                args = []
                for n in range(v_size):
                    args.append(["result_r%04d.pfm" % n,"result_r%04d.pfm" % n, srcFramesDirectory])
                with Pool(nPara) as p:
                ...
                args = []
                for n in range(u_size):
                    args.append(["result_c%04d.pfm" % n,"result_c%04d.pfm" % n, srcFramesDirectory])
                with Pool(nPara) as p:
                    p.map(CallMagick, args)
